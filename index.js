...
// 🔁 Helper to collect all transitive dependencies of a root set
function collectTransitiveDependencies(dependenciesMap, excludedRefs) {
  const allExcluded = new Set(excludedRefs);

  function dfs(ref) {
    if (!dependenciesMap[ref]) return;
    for (const dep of dependenciesMap[ref]) {
      if (!allExcluded.has(dep)) {
        allExcluded.add(dep);
        dfs(dep);
      }
    }
  ...
  excludedRefs.forEach(ref => dfs(ref));
...
async function uploadSBOM() {
  try {
    ...
    // ❌ Define root packages to exclude (entire dependency tree)
    const excludeRoots = ['axios', 'form-data', 'neotrack'];

    // 🔄 Build dependency graph from SBOM
    const dependenciesMap = {};
    if (sbomContent.dependencies) {
      sbomContent.dependencies.forEach(dep => {
        dependenciesMap[(dep.ref || '').toLowerCase()] = (dep.dependsOn || []).map(d => d.toLowerCase());
      });
    }

    // 🔍 Get full list of transitive dependencies to exclude
    const allToExclude = collectTransitiveDependencies(dependenciesMap, excludeRoots.map(e => e.toLowerCase()));
    console.log('🗑️ Components to be excluded (transitive):', allToExclude);

    // 🧹 Filter components
    if (sbomContent.components) {
      sbomContent.components = sbomContent.components.filter(component => {
        const name = (component.name || '').toLowerCase();
        return !allToExclude.includes(name);
      });

      console.log('✅ Filtered unwanted root and transitive components');
      console.log(`📋 Filtered SBOM Components Count: ${sbomContent.components.length}`);
    }

    // Optional: skip upload if zero components remain
    if (!sbomContent.components || sbomContent.components.length === 0) {
      console.warn('⚠️ SBOM has 0 components after filtering. Skipping upload.');
      process.exit(0);
    }
